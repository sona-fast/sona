...
import { stubTurnstile, STUB_TOKEN_PREFIX } from './admin-login';
...
test('wrong password shows the error, re-issues the token, and a retry logs in', async ({
	...
	const stubToken = new RegExp('^' + STUB_TOKEN_PREFIX);
	const tokenInput = page.locator('input[name="cf-turnstile-response"]');
	await expect(tokenInput).toHaveValue(stubToken, { timeout: 15_000 });
	...
	await expect(tokenInput).toHaveValue(stubToken);
