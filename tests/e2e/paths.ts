...
// point the dev server at the throwaway DB) and global-setup.ts (to build it),
// so the two processes agree on which config + persist dir to use.
...
// global-setup seeds via the CLI (E2E_PERSIST_TO); the dev server reads via
