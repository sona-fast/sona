...
describe('conCardFaceSvg — the front', () => {
	...
	it('carries the name and the species, and nothing else the back owns', () => {
		const svg = front({ artCredit: 'Ref sheet by @nori', handles: [{ platform: 'bluesky', value: '@taro' }] });
		...
		expect(texts(svg)).not.toContain('Ref sheet by @nori');
...
describe('conCardFaceSvg — the back', () => {
	...
	it('signs the bottom edge with the art credit and the made-with line', () => {
		const svg = back({ artCredit: 'Ref sheet by @nori · taro.surf' });
		expect(texts(svg)).toEqual(
			expect.arrayContaining(['Ref sheet by @nori · taro.surf', 'Made with Sona'])
		);
	});

	it('drops the credit line when the art credit is turned off', () => {
		const svg = back({ artCredit: null });
		expect(texts(svg)).not.toContain('Ref sheet by @nori · taro.surf');
...
describe('conCardFaceSvg — handles', () => {
	...
	it('caps the rows before the last one reaches the microtext', () => {
		...
		const svg = back({
			...
			artCredit: 'Ref sheet by @nori'
		});
		const rows = [...svg.matchAll(/<text [^>]*y="(\d+)"[^>]*>(@row\d+)<\/text>/g)].map((m) => ({
			y: Number(m[1]),
			body: m[2]
		}));
		const creditY = Number(svg.match(/<text [^>]*y="(\d+)"[^>]*>Ref sheet by[^<]*<\/text>/)?.[1]);
