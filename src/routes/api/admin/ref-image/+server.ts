...
// the DB — this endpoint never fetches a caller-supplied URL (SSRF). The rest
// of the hardening (private hosts, redirects, content types) lives in
// $lib/server/image-proxy.ts, shared with the avatar endpoint.
...
export const GET: RequestHandler = async ({ url, platform, fetch }) => {
	...
	const response = await proxyStoredImage(row.imageUrl, fetch);
	if (!response) error(502, 'Could not fetch the stored image');
	return response;
};
