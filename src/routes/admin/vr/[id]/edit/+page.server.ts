...
import { artists, characters, images, vrAvatars, avatarCredits, avatarPlatforms } from '$lib/server/db/schema';
import { parseAvatarForm, validateAvatarRefs, updateAvatar, deleteAvatar } from '$lib/server/vr-avatars';
...
export const load: PageServerLoad = async ({ params, platform }) => {
	...
	const [credits, platforms, allArtists, allCharacters, allImages, publishingEnabled] =
		await Promise.all([
			db
...
export const actions = {
	save: async ({ params, request, platform }) => {
		...
		if (parsed.input.published && !existing.published && !(await vrPublishingEnabled(db))) {
			return fail(403, { error: 'VR avatars is in early access — publishing needs a valid supporter key until it opens for everyone.' });
		}
